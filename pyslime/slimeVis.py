...
def add_points(coords,slimeobj):
    ...
    cenidxs = slimeobj.cartesian_to_idx(cartcoords[0], cartcoords[1], cartcoords[2])
...
def bokeh_this_that(tab, colx, coly, cds=None, tooltip=None, title=None,
    ...
    if view is not None:
        fig.square(colx, coly, source=cds, size=markersize, view=view, fill_alpha=alpha, **kwargs)
    else:
        fig.square(colx, coly, source=cds, size=markersize, fill_alpha=alpha, **kwargs)
...
def inspect3d(slimeobj,coords,z,boxsize=[20,20,20],windowsize=[900,900],
    ...
    cenidxs = sliced.cartesian_to_idx(cartcoords[0], cartcoords[1], cartcoords[2])
    ...
    if otherObjectRaDecRed is not None:
        othercoords = SkyCoord(otherObjectRaDecRed[0], otherObjectRaDecRed[1],
                               distance=cosmo.luminosity_distance(otherObjectRaDecRed[2]),
                               unit=('deg', 'deg', 'Mpc'))
